...
$(document).ready(function(){
    var userID = $('#hidden-id').html();
    $('table#my-activities').hide();
    $.post('/api/user-activities', {id: userID, limit: 100}, function(res) {
...
function makeTableElement(data, cb) {
    var res;
    var screenName = $('#hidden-screen-name').html();
    switch(data.type) {
    case 'single':
        res = '<td><div><div>' + screenName + 'のコポり: ' +
            '<strong>' + data.points + 'pt獲得！</strong>' +
            '</div><div>' + formatedDate(data.user_screen_name, data.status_id, new Date(data.date)) +
            '</div></div></td>';
        break;
    case 'post_reply':
        res = '<td><div><div>' + screenName + 'の' + formatedScreenName(data.target_screen_name) + 'へのコポり: ' +
            '<strong>' + data.points + 'pt獲得！</strong>' +
            '</div><div>' + formatedDate(data.user_screen_name, data.status_id, new Date(data.date)) +
            '</div></div></td>';
        break;
